...
- Si possible, monter l'ISO `virtio-win.iso` présente dans le dossier nouvellement généré de Quickemu pour installer tous les pilotes VirtIO ainsi que les services SPICE.
  Si cette étape n'a pas été effectuée, il est possible d'ajouter les drivers à posteriori (voir plus loin).
...
Windows ne pourra pas détecter le disque C: si les pilotes VirtIO ne sont pas présents.  
...
- Lancer Quickemu avec les arguments suivants pour monter l'image VirtIO et démarrer depuis l'image d'installation de Windows : `quickemu --vm windows-10-French.conf --extra_args "-drive media=cdrom,index=2,file=windows-10-French/virtio-win.iso -boot order=d"`.
...
- L'image CD VirtIO doit être assignée au lecteur `E:`. Si ce n'est pas le cas, mettre le bon lecteur dans les commandes suivantes.
- Charger le pilote VirtIO Storaage qui permettra à Windows détecter le disque dur. Ce pilote est chargé à chaud dans l'installateur de Windows (Win RE). Taper `drvload E:\viostor\w10\amd64\viostor.inf`.
...
- Taper `list volume` pour déterminer quel est le nom de volume temporaire correspondant au `C:` du disque dur. Dans l'exemple suivant, ce sera `F:`. Taper `exit` pour quitter `diskpart`.
