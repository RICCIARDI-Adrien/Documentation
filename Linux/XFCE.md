...
## Désactiver le lock automatique après inactivité

Supprimer light-locker et rebooter (ou tuer tous les processus light-locker).
...
## Support format WEBP dans Ristretto

Installer le paquet `webp-pixbuf-loader`.
...
Méthode fonctionnant sur Xubuntu 19.04 64 bit avec une GeForce 1070 utilisant les drivers propriétaires version 418 :
* Activer le compositeur
* Activer le Composition Pipeline à l'aide de nvidia-settings.
...
Créer le dossier de configuration s'il n'existe pas encore :
```
mkdir -p ~/.config/picom
```

Copier le fichier de configuration suivant (inspiré du template dans le code source) : `nano ~/.config/picom/picom.conf`
```
#################################
#             Shadows           #
#################################
...
Désactiver le compositeur par défaut :
```
xfconf-query -c xfwm4 -p /general/use_compositing # (doit retourner true s'il est activé)
xfconf-query -c xfwm4 -p /general/use_compositing -s false
xfconf-query -c xfwm4 -p /general/use_compositing # (doit maintenant retourner false)
```

Il ne reste plus qu'à se déconnecter de la session et à s'y reconnecter pour utiliser le nouveau compositeur.

Source : https://wiki.manjaro.org/index.php/Using_Compton_for_a_tear-free_experience_in_Xfce/fr